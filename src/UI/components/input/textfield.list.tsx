...
export const TextfieldList: React.FC<TextfieldListProps> = ({
  ...
  return (
    <div className="flex flex-col w-full relative">
      ...
      <div
        className="flex border border-solid bg-white border-blue-sky justify-between items-center rounded-lg p-3 cursor-pointer"
      ...
      {isOpen && (
        <Modal onClose={() => setIsOpen(false)} fullWidth>
          {valuesToDisplay.map((element, index) => (
            <Cell key={index} onClick={() => handleCellClick(element)}>
              {element}
            </Cell>
          ))}
        </Modal>
