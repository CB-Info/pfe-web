...
import {
  Ingredient,
  IngredientUnity,
} from "../../../../data/models/ingredient.model";
...
export const IngredientsModal: React.FC<IngredientModal> = ({
  ingredientsDisplay,
  currentIngredients,
  ...
  const filteredIngredients =
    ingredientsDisplay.length > 0
      ? ingredientsDisplay.filter((ingredient) =>
          ingredient.name
            .toLowerCase()
            .includes(state.searchIngredientInput.toLowerCase())
        )
      : [];
  ...
  function onClickOnUnity(unity: IngredientUnity) {
    ...
    if (currentIngredient != undefined) {
      ...
      dispatch({
        ...
        payload: { ...currentIngredient, unity: unity },
      });
    }
    dispatch({
        ...
        <>
          ...
          >
            <ModalInput
              ...
              value={state.userSearchUnity}
              onChange={(e) =>
                dispatch({
                  type: "SET_USER_SEARCH_UNITY",
                  payload: e.target.value,
                })
              }
            />
            <div className="flex flex-col py-1.5 gap-2">
              {(isInputInIngredients ||
                  ...
                  {Object.values(IngredientUnity).map((element, index) => {
                    return (
                      <CellButton
                        key={index}
                        onClick={() => {
                          onClickOnUnity(element);
                        }}
                        label={toCapitalize(element.toString())}
                      />
                    );
                  })}
                </div>
