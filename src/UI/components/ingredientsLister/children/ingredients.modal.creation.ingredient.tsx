...
export const IngredientsModalCreation: React.FC<IngredientModalCreation> = ({
  currentIngredients,
  ...
  function handleOnChangeIngredientValue(e: ChangeEvent<HTMLInputElement>) {
    ...
    if (currentIngredient != undefined) {
      ...
      dispatch({
        ...
        payload: { ...currentIngredient, value: inputValue },
      });
    }
  }
  ...
  return (
    <>
      {state.currentIngredient != undefined && (
        <Modal
          onClose={handleOnCloseCreationDishModal}
          position={PositionModal.TOP}
        >
          <ModalInput
            placeholder="Nom de l'ingrédient"
            value={state.userIngredientNameInput}
            onChange={(e) =>
              dispatch({
          ...
          >
            ...
            <div className="flex gap-1">
              <span>
                {toCapitalize(state.currentIngredient.unity.toString())}
              </span>
          ...
          >
            ...
            <div className="flex">
              <ModalInput
                placeholder="Valeur de l'ingrédient"
                value={state.userIngredientValueInput}
                onChange={handleOnChangeIngredientValue}
                type="number"
              />
            </div>
          </CellCLStyle>
        ...
        >
          <div className="flex flex-col flex-1 justify-center items-center px-6 gap-2">
            <span className="font-inter text-base text-center px-1">
              Souhaitez-vous supprimer cet ingrédient de votre plat ?
            </span>
            <div className="flex flex-col w-full">
              <ModalButton
                isErrorColors={true}
                onClick={handleDeleteLocallyIngredient}
              >
                Supprimmer uniquement pour ce plat
              </ModalButton>
              <ModalButton
                onClick={() => {
                  setIsConfirmationModalOpen(false);
