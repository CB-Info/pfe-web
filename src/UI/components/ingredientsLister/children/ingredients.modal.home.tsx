...
import {
  NavigationModal,
  useIngredientsListerDispatchContext,
  useIngredientsListerStateContext,
} from "../ingredients.lister.reducer";
import { CellButton } from "../../buttons";
...
export const IngredientsModalHome: React.FC<IngredientModalHome> = ({
  ingredientsDisplay,
  currentIngredients,
  ...
  useEffect(() => {
    if (state.suggestIngredients.length == 0) {
      dispatch({
        type: "UPDATE_SUGGEST_INGREDIENTS",
        payload: ingredientsDisplay,
      });
    }
  }, []);
  ...
  useEffect(() => {
    if (state.searchIngredientInput != "") {
      const fetchIngredients = async () => {
        if (state.searchIngredientInput) {
          try {
            const response = await ingredientRepositoryImpl.getByName(
  ...
  async function handleOnClickOnNewIngredient() {
    try {
      const ingredientDto: IngredientDto = {
        _id: "",
        name: state.searchIngredientInput,
      };
      const newIngredient = await ingredientRepositoryImpl.createOne(
        ingredientDto
      );
      reset();
      ...
      dispatch({
        type: "SET_USER_INGREDIENT_NAME_INPUT",
        payload: newIngredient.name,
      });
      dispatch({ type: "UPDATE_CURRENT_INGREDIENT", payload: newIngredient });
      dispatch({
        type: "SET_NAVIGATION_MODAL",
        payload: NavigationModal.CREATION_INGREDIENT,
      });
    } catch (error) {
      console.log(error);
    }
  ...
  function handleOnIngredient(ingredient: Ingredient) {
    ...
    dispatch({
      type: "SET_USER_INGREDIENT_NAME_INPUT",
      payload: ingredientToUse.name,
    });
    dispatch({ type: "UPDATE_CURRENT_INGREDIENT", payload: ingredientToUse });
    dispatch({
      type: "SET_USER_INGREDIENT_VALUE_INPUT",
      payload: ingredientToUse.value?.toString() ?? "",
    });
    dispatch({
      type: "SET_NAVIGATION_MODAL",
      payload: NavigationModal.CREATION_INGREDIENT,
  ...
  return (
    <Modal
      ...
      position={PositionModal.TOP}
    >
      <>
        <ModalInput
          placeholder="Rechercher un  ingrédient ou ajouter un nouvel ingrédient"
          value={state.searchIngredientInput}
          onChange={onChangeInput}
        />
        <div className="flex flex-col py-1.5 gap-2">
          {state.searchIngredientInput.length != 0 ? (
            <div className="flex flex-col">
              <span className="font-inter font-semibold text-xs px-3.5 cursor-default pb-2">
                Ingrédients
              </span>
              {ingredientsDisplay.length > 0 ? (
                ingredientsDisplay.map((ingredient) => (
                  <CellButton
                    key={ingredient.name}
                    onClick={() => handleOnIngredient(ingredient)}
            ...
            <div className="flex flex-col">
              ...
              {state.suggestIngredients.length > 0 ? (
                state.suggestIngredients.map((ingredient) => (
                  <CellButton
                    key={ingredient.name}
                    onClick={() => handleOnIngredient(ingredient)}
                    label={ingredient.name}
          ...
          {state.searchIngredientInput.length != 0 &&
            !ingredientsDisplay.find(
              (e) =>
                e.name.toLowerCase() ===
                state.searchIngredientInput.toLowerCase()
            ) && (
              <div className="flex flex-col">
                <span className="font-inter font-semibold text-xs px-3.5 cursor-default pb-2">
                ...
                <CellButton
                  onClick={handleOnClickOnNewIngredient}
                  label={state.searchIngredientInput}
                />
              </div>
            )}
        </div>
